...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
      v_sum_adj = sum_node_list(node_to_output_grads_list[i])
      i.grad = v_sum_adj
      if i.op:  
        c = i.op.gradient_as_tuple(i.grad, i)
        for k, vki in zip(i.inputs, c):
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for i in node_list:
        if i not in visited:
           visited, topo_order = topo_sort_dfs(i, visited, topo_order)
    return topo_order
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for neighbor in node.inputs:
        if neighbor not in visited:
         visited, topo_order = topo_sort_dfs(neighbor, visited, topo_order)
