...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    for (int  dim = 0; dim <shape.size; dim++) {
      prod = prod*shape.data[dim];
    }
    for (int  dim = 0; dim <shape.size; dim++) {
        prod = prod/shape.data[dim];
        int currentDimIndex = (linearIndex / prod);
        index += currentDimIndex * strides.data[dim];
        linearIndex = linearIndex%prod;
    }
    //printf("index:\n%s\n", index);
    // Copy the value from 'a' to 'out' with the calculated index
    out[gid] = a[index];
  }
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
   ...
   * compact input a, to the corresponding item in the non compact array out.
   * 
   * Args:
   *   a: CUDA pointer to a array
   *   out: CUDA point to out array
  ...
  if (gid < size) {
    ...
    for (int  dim = 0; dim <shape.size; dim++) {
        prod = (int) prod/shape.data[dim];
        int currentDimIndex = (int)(linearIndex / prod);
        index += currentDimIndex * strides.data[dim];
        linearIndex = linearIndex%prod;
    }
    //printf("index:\n%s\n", index);
    // Copy the value from 'out' to 'a' with the calculated index
    out[index] = a[gid];
  }
}
  //assert(false && "Not Implemented");
...
__global__ void ScalarSetitemKernel(const scalar_t val, scalar_t* out,  size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    for (int  dim = shape.size-1; dim >=0; dim--) {
        ...
        int currentDimIndex = (linearIndex / prod);
        index += currentDimIndex%shape.data[dim] * strides.data[dim];
        // linearIndex = linearIndex%prod;
        prod = prod*shape.data[dim];
    }
    //printf("index:\n%s\n", index);
    // Copy the value from 'val' to 'out' with the calculated index
    out[index] = val;
  }
}
...
void ScalarPower(const CudaArray& a, scalar_t val, CudaArray* out) {
  /**
   * raise a CUDA array by a scalar value.
   */
  CudaDims dim = CudaOneDim(out->size);
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b,scalar_t* out, uint32_t M, uint32_t N,
            uint32_t P){
    // Calculate global indices for the output matrix
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 grid_dim((P + block_dim.x - 1) / block_dim.x, (M + block_dim.y - 1) / block_dim.y);
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
   ...
   */
  /// BEGIN SOLUTION
  // size_t num_blocks = (out.size) / reduce_size;
  CudaDims dim = CudaOneDim(out->size);
